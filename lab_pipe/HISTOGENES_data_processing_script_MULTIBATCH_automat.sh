...
for run in 220603_K00233_0273_BHNN3NBBXY_SRdi_SN_JR \
 220719_K00233_0283_BHNMWWBBXY_SRdi_SN_JR \
 220627_K00233_0276_AHNMY2BBXY_SRdi_BN \
 220621_K00233_0275_BHNYMKBBXY_SRdi_BN \
 220630_K00233_0277_BHNMWKBBXY_SRdi_BN_JR_SN
do
    list+=${run}" "
    echo $run
    fqpt="/mnt/archgen/data_releases/2022/"${run}
    listfile=known_anno_b${nbtch}_${run}.txt

    cp ${fqpt}/known_anno.txt ${listfile}


    grep -e HistoGenes -e FORMOR -e Slavs -e Hofmanova known_anno_b${nbtch}_${run}.txt \
     | grep -e \\.TF -e \\.SG \
     | cut -f2 \
     | sort | uniq \
     | sed 's/\.SG.*//g' \
...
#     grep -e HistoGenes -e FORMOR -e Slavs -e Hofmanova ${listfile} | grep -e \\.TF -e \\.SG \
#      | cut -f 2 \
#      | cut -c 1-3 \
#      | sort \
...
#     echo \
#      " sed s/\".TF1.1\"/\"\"/g | sed s/\".TF2.1\"/\"\"/g | sed s/\".TF1.2\"/\"\"/g | cat - | cut -f 1 | tail -n +1 | sort -k1,1 | uniq  > bbb1" \
#      >> cmd.tmp.sh
#     sed -i ':a;N;$!ba;s/|\n/|/g' cmd.tmp.sh
...
grep -v old bbb2 | grep -v bug > ${idf}
rm bbb*
